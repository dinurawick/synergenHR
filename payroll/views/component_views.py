...
def payroll_calculation(employee, start_date, end_date):
    ...
    # Update total_deductions to include net pay deductions (excluding employer contributions)
...
def get_contribution_report(request):
    ...
    if employee_id:
        ...
        deductions.sort(key=lambda x: x["title"])  # Sort by title instead of deduction_id
        grouped_deductions = {
            key: list(group)
            for key, group in groupby(deductions, key=lambda x: x["title"])  # Group by title
        }

        for title, group in grouped_deductions.items():
            deduction_id = group[0]["deduction_id"]
            # Calculate employee contribution (exclude employer contributions)
            employee_contribution = sum(
                item.get("amount", 0) for item in group 
                if not item.get("is_employer_contribution", False)
            )
            
            # Check if we have new format employer contributions
            has_new_format = any(item.get("is_employer_contribution", False) for item in group)
            
            if has_new_format:
                # Use only new format - ignore old employer_contribution_amount
                employer_contribution = sum(
                    item.get("amount", 0) for item in group 
                    if item.get("is_employer_contribution", False)
                )
            else:
                # Use old format only if no new format present
                employer_contribution = sum(
                    item.get("employer_contribution_amount", 0) for item in group
                )
            
            total_contribution = employee_contribution + employer_contribution
            if employer_contribution > 0 or employee_contribution > 0:  # Show if either has value
