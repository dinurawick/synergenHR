...
def contract_ending(request):
    ...
    # Use the selected month/year directly, no special logic for initialLoad
    contract_end = Contract.objects.filter(
        contract_end_date__month=int(month), 
        contract_end_date__year=int(year)
    )
...
def employees_joined(request):
    ...
    response = {
        ...
        "message": _("No employees joined this month"),
    }
    return JsonResponse(response)
