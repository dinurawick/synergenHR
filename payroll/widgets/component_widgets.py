...
class DeductionConditionalVisibility(forms.Widget):
    ...
    def render(self, name, value, attrs, renderer) -> SafeText:
        ...
        import time
        import random
        cache_buster = f"{int(time.time())}{random.randint(1000, 9999)}"  # More aggressive cache busting
        rendered_script = (
            f'<script src="/{settings.STATIC_URL}build/js/deductionWidget.js?v={cache_buster}"></script>'
        )
        additional_script = f"""
        <script id="{name}Script">
            $(document).ready(function () {{
