    ...
    for deduction in deductions:
        ...
        amount = calculation_function(
            **{
                ...
                "total_allowance": kwargs["total_allowance"],
                "basic_pay": kwargs["basic_pay"],
                "gross_pay": kwargs.get("gross_pay", 0),  # Add gross_pay
                "day_dict": kwargs["day_dict"],
            }
        )
        kwargs["amount"] = amount
...
def if_condition_on(*_args, **kwargs):
    ...
    if not isinstance(component, Allowance):
        # Use gross_pay from kwargs if available, otherwise calculate
        gross_pay = kwargs.get("gross_pay", 0)
        if not gross_pay:
    ...
    if component.if_condition == "range":
        ...
        if operator_func and not operator_func(condition_value, component.if_amount):
            amount = 0
...
def calculate_based_on_gross_pay(*_args, **kwargs):
    ...
    component = kwargs["component"]
    day_dict = kwargs["day_dict"]
    # Use the gross_pay passed in kwargs instead of recalculating
    gross_pay = kwargs.get("gross_pay", 0)
    if not gross_pay:
        # Fallback to calculation if not provided
        gross_pay_data = calculate_gross_pay(**kwargs)
        gross_pay = gross_pay_data["gross_pay"]
