        ...
        if deductions:
            for deduction in deductions:
                if deduction.get("deduction_id"):
                    ...
                    if object and object.employer_rate > 0:
                        # Get the base amount for calculation
                        if object.based_on:
                            amount = pay_head_data.get(object.based_on)
                            # Handle case where amount is None (e.g., for net_pay based deductions)
                            if amount is None:
                                # Try to get the amount from the main data dictionary
                                amount = data.get(object.based_on, 0)
                        else:
                            # If based_on is None or empty, use the deduction amount itself as base
                            # This is common for net pay deductions where the employee contribution
                            # is calculated first, then employer contribution is based on the same base
                            amount = deduction.get("amount", 0)
                            # For net pay deductions, we need to reverse-calculate the base amount
                            # If employee paid 8% and got amount X, then base = X / 0.08
                            if object.rate > 0:
                                amount = (float(amount) * 100) / float(object.rate)
                        
                        # Ensure amount is not None and is a valid number before calculation
                        if amount is not None and isinstance(amount, (int, float)) and amount > 0:
                            employer_contribution_amount = (
                                float(amount) * float(object.employer_rate)
                            ) / 100
